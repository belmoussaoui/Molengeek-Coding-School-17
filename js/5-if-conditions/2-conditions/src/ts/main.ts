...
let r2 = "parseint";
let score: number = +(r1 == q1.toLowerCase()) + +(r2 == q2.toLowerCase());
...
if (isNaN(age)) alert("Vu que vous ne savez pas lire une instruction, je présume que vous n'avez pas l'âge.");
else if (age < 0) alert("Revenez quand vous serez né !");
else if (age === 18) alert("Réservez mais attention, c'est tout juste et ce film peut être choquant !");
else if (age > 18) alert("vous êtes majeur, vous pouvez réserver.");
else alert("vous êtes mineur, revenez plus tard.");
...
if (!res1) alert("Pas de problème, ne partez pas en vacances.");
if (!res2 || res3) alert("Même si vous le vouliez, vous ne pouriez pas partir...");
if (res2 && !res3) alert("Vous pourriez partir en vacance si vous le vouliez.");
if (res1 && res2 && !res3) alert("Tout est parfait, partez en vacance !");
if (!(!res1 || !res2 || res3)) alert("Tout est parfait, partez en vacance !");
