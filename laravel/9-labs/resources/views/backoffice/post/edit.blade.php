...
@section('content')
    ...
    <form class="form-class" action="{{ route("posts.update", $post) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="mb-3">
            <label class="form-label">Title</label>
            <input class="text-black" type="text" name="title" value="{{ $post->title }}">
        </div>
        <div class="mb-3">
            <label class="form-label">Content</label>
            <textarea class="text-black" name="content">{{ $post->content }}</textarea>
        </div>
